...
import static com.blocklaunch.blwarps.data.WarpDataQueries.GROUPS;
...
public class WarpBuilder implements DataBuilder<Warp> {
    ...
    public Optional<Warp> build(DataView container) throws InvalidDataException {
        if (container.contains(NAME, WORLD, X, Y, Z, GROUPS)) {
            Warp warp = new Warp(
            ...
            warp.setGroups((List<String>) container.getList(GROUPS).get());
