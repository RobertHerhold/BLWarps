...
public class WarpGroupCommand implements CommandExecutor {
	...
	private static final Text NO_OPERATION_MSG = Texts.of(TextColors.RED,
			BLWarps.PREFIX
					+ " You need to specify an operation: ADD, REMOVE, REMOVEALL");
	private static final Text SPECIFY_WARP_MSG = Texts.of(TextColors.RED,
			BLWarps.PREFIX + " You must specify a warp!");
	private static final Text SPECIFY_GROUP_MSG = Texts.of(TextColors.RED,
			BLWarps.PREFIX + " You must specify a group!");
	private static final Text NO_WARPS_AFFECTED = Texts.of(TextColors.RED,
			BLWarps.PREFIX + " No warps were affected!");
	...
	class DeleteGroupExecutor implements CommandExecutor {
		...
		public CommandResult execute(CommandSource source, CommandContext args)
			...
			source.sendMessage(Texts.of(TextColors.GREEN, BLWarps.PREFIX
					+ " The ", TextColors.GOLD, group, TextColors.GREEN,
					" group tag was removed from ", TextColors.GOLD,
					affectedCounter, TextColors.GREEN, " warps."));

			return CommandResult.builder().successCount(affectedCounter)
					.build();
		}
	...
	class AddWarpToGroupExecutor implements CommandExecutor {
		...
		public CommandResult execute(CommandSource source, CommandContext args)
			...
			source.sendMessage(Texts.of(TextColors.GREEN, BLWarps.PREFIX
					+ " You successfully added ", TextColors.GOLD,
					warp.getName(), TextColors.GREEN, " to ", TextColors.GOLD,
					group, TextColors.GREEN, "."));
	...
	class RemoveWarpFromGroupExecutor implements CommandExecutor {
		...
		public CommandResult execute(CommandSource source, CommandContext args)
			...
			if (!warp.getGroups().contains(group)) {
				source.sendMessage(Texts.of(TextColors.RED, BLWarps.PREFIX + " ",
						TextColors.GOLD, warp.getName(), TextColors.RED,
						" is not in the group ", TextColors.GOLD, group,
						TextColors.RED, "."));
				return CommandResult.empty();
			}
			...
			source.sendMessage(Texts.of(TextColors.GREEN, BLWarps.PREFIX
					+ " You have successfully removed ", TextColors.GOLD,
					warp.getName(), TextColors.GREEN, " from ",
					TextColors.GOLD, group, TextColors.GREEN, "."));
